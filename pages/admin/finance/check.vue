<template>
	<view>
		<cu-custom ref="cust" bgColor="bg-white" :isBack="true" :isHome="false"><block slot="content">买单记录</block></cu-custom>
		<view class="">
			<view class="header">
				...
				<view class="flex justify-between bg-white top-screen">
					...
					<view class="flex align-center" @click="changeTime()">
						买单时间<text class="icon cuIcon-order"  :class="timeUp?'text-gold':''"></text>
					</view>
				</view>
		</view>
		<!-- list -->
		<view class="list">
			<view class="item bg-white" style="border-radius:0rpx;margin:20rpx;padding:20rpx 0rpx 20rpx 0"
			v-for="(item,index) in userList" :key="index">
				<view class="people flex justify-start">
					...
					<view class="flex right-part justify-around" style="margin-left: 20rpx;">
						<view class="grid col-1" style="width: 60%;">
							<view class="name text-bold">{{item.nickname}}({{item.user_id}})</view>
							<view class="something" v-show="index %2 ==0">《艺伎回忆录》</view>
							<view class="something">{{item.time}} 微信支付</view>
							<view class="flex align-center" style="margin-top: 10rpx;">
								<view class="cu-btn bg-black daifu">代付人</view> <view class="daifu_name text-bold">Enzo</view>
							</view>
						</view>
						<view class="grid col-1" style="text-align: right;">
							<view class="something">实际支付 <text class="text-orange">99</text> 元</view>
							<view class="something">金额99元</view>
						</view>
					</view>
				</view>
			</view>
